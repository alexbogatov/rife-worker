...
# 2. Install only essential system binaries (FFmpeg, Git, Curl)
...
# 4. Install CUDA-enabled PyTorch (CUDA 12.1 runtime wheels) without pip cache
RUN pip install --no-cache-dir --upgrade pip \
    && pip install --no-cache-dir \
    torch torchvision --index-url https://download.pytorch.org/whl/cu121

    # Prune unnecessary PyTorch/CUDA distributed libraries (saves ~2.5 GB)
RUN find /usr/local/lib/python3.10/site-packages/torch/lib/ \
    -name "libnccl*" -o \
    -name "libcufft*" -o \
    -name "libcurand*" -o \
    -name "libcusparse*" \
...
# 5. Clone ComfyUI Core & only the necessary custom nodes for RIFE
...
# Copy entrypoint and worker files
COPY . .
